    ...
    <div class="col-12 position-relative">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Peta Monitoring Karbon</h3>
        </div>
...
.info-panel tfoot td {
  background-color: #f7fdf7; font-weight: 600;
}
</style>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderPlotAreas(areas) {
    ...
    areas.forEach(a => {
      ...
      const m = L.circleMarker([lat, lng], {
        radius: 8, fillColor: '#28a745', color: '#fff',
  ...
  fetch(`/zones/${slug}/summary`)
    ...
    .then(res => {
      ...
      const rows = [
        { label: 'Serasah',              co2: parseFloat(obj.Serasahco2),           luas: parseFloat(obj.faktor), total: parseFloat(obj.Serasah),             persen: parseFloat(obj.hasilSerasahPersen) },
        { label: 'Necromass',            co2: parseFloat(obj.Necromassco2),         luas: parseFloat(obj.faktor), total: parseFloat(obj.Necromass),           persen: parseFloat(obj.hasilNecromassPersen) },
        { label: 'CO₂ Tanaman',     co2: parseFloat(obj.Co2Tanaman),           luas: parseFloat(obj.faktor), total: parseFloat(obj.TotalCarbonn),        persen: parseFloat(obj.hasilco2tanamanPersen) },
        { label: 'Tanah',                co2: parseFloat(obj.TanahCo2),             luas: parseFloat(obj.tanah),   total: parseFloat(obj.TanahCarbon),        persen: parseFloat(obj.hasiltanahPersen) },
        { label: 'Berat biomasa akar',   co2: parseFloat(obj.BeratBiomassaAkar),    luas: parseFloat(obj.faktor), total: parseFloat(obj.beratMasaAkar),     persen: parseFloat(obj.hasilakarPersen) },
      ];
