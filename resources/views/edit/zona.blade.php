        ...
        <div class="container-isi">
            <div class="card plot-info-card">
                ...
                <div class="card-body">
                    ...
                    <!-- Form -->
                    <dd></dd>
                    <form method="POST" action="{{ route('zona.update', ['slugP' => $poltArea->slug, 'slugZ' => $zona->slug]) }}" id="plotAreaForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select  form-control" aria-label="Default select example" name="zona">
                                <option selected value="{{ $zona->zona}}">{{ $zona->zona}}</option>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control-non" name="latitude" id="latitude"  value="{{ old('latitude', $zona->latitude) }}" />
                        </div>
                        <div class="mb-4">
                            <label for="longitude" class="form-label">Longitude</label>
                            <input type="text" class="form-control-non" name="longitude" id="longitude"  value="{{ old('longitude', $zona->longitude) }}"/>
                        </div>

                        <div class="mb-4">
                            <label for="plotName" class="form-label">Jenis hutan</label>
                            <select class="form-select  form-control" aria-label="Default select example"
                                name="jenis_hutan" value="{{ $zona->jenis_hutan}}">
                                <option selected value="{{ $zona->jenis_hutan}}">{{ $zona->jenis_hutan}}</option>
